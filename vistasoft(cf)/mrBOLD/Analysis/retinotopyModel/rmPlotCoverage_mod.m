function [RFcov figHandle all_models weight data] = rmPlotCoverage_mod(vw, varargin)
...
load('/Users/Nicolas/Google Drive/MWE/Gray/GLM-betas/EITWOVEIONE.mat')

EImap = cell2mat(map);

%normBetas = (EImap - min(EImap)) / ( max(EImap) - min(EImap) );
%weight = weight .* normBetas(coIndices); %% normalized between 0-1
weight = weight .* EImap(coIndices);
...
% no bootstrap
    ...
    switch lower(vfc.method)
        ...
        case {'maximum profile', 'max', 'maximum'}
            ...
            figure,plot(all_models_weighted);
