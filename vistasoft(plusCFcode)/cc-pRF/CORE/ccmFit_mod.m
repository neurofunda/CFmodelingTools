function params = ccmFit_mod(view,params)
...
params.analysis.allstimimages = single(params.analysis.allstimimages);
    ...
    %% Fig before detrending source (3)
    % figure,plot(params.analysis.allstimimages);
    % xlabel('Time points','FontSize', 15);
    % title('Source before detrending ');
...
% %% Window for detrended time series
%     if params.win == 1
%         
%         X = [1:1:length(allstimimages(:,1))]';
%         W = taper(X,'TaperPar',0.5);
%         %
% %         figure,plot(win);
%       %   figure,subplot(211);plot(params.analysis.allstimimages);
%         for i = 1:length(allstimimages(1,:));
%             allstimimages(:,i) = allstimimages(:,i).*W;
%         end
%        %  subplot(212);plot(params.analysis.allstimimages);
%     end
% 

    % figure,plot(allstimimages);
    % xlabel('Time points','FontSize', 15);
    % title('Source after detrending ');
    % ylabel('BOLD %','FontSize', 15);
    % set(gcf, 'color', 'w');
    % set(gca,'LineWidth',1);
    % set(gca, 'FontSize', 15);
...
prediction = allstimimages(:,good_voxels)*RF;
    ...
    % xlabel('Time points','FontSize', 15);
    % ylabel('BOLD signal change Prediction (%)','FontSize', 15);
    % set(gcf, 'color', 'w');
    % set(gca,'LineWidth',1);
    % set(gca, 'FontSize', 15);
...
[data, params] = ccmLoadData(view, params, params.targetROI);
    ...
    % figure,plot(data); 
    % xlabel('Time points','FontSize', 15);
    % ylabel('BOLD signal change (%)','FontSize', 15);
    % title('Target before detrending ');
    % set(gcf, 'color', 'w');
    % set(gca,'LineWidth',1);
    % set(gca, 'FontSize', 15);

%% Fig target before detrending (5)
%figure,plot(data); 
...
% figure,plot(trends);
%% Fig trendBetas
% figure,plot(trendBetas);

% %% Window for detrended target
% if params.win == 1
%     
%     X = [1:1:length(data(:,1))]';
...
% end
    ...
    % %% Fig target after detrending (5)
    % figure,plot(data); 
    % xlabel('Time points','FontSize', 15);
    % title('Target after detrending ');
    % ylabel('BOLD signal change (%)','FontSize', 15);
    % set(gcf, 'color', 'w');
    % set(gca,'LineWidth',1);
    % set(gca, 'FontSize', 15);
...
for n=1:numel(params.analysis.pRFmodel)
    ...
    %% no detrending (ccmFit_core itself has been modified...
%     s{n}=ccmFit_core_mod(s{n},prediction,data,params);
